class cikl_logstash::config (
  ...
  file { 'elasticsearch-cikl-template': 
    ...
    group   => "root",
    mode    => '0644',
    content => template('cikl_logstash/elasticsearch-cikl-template.json.erb')
  }

  ::logstash::configfile { 'input-rabbitmq':
    content => template('cikl_logstash/logstash-input-rabbitmq.conf.erb'),
    order   => 10
  }

  ::logstash::configfile { 'filter-event':
    content => template('cikl_logstash/logstash-filter-event.conf.erb'),
    order   => 20
  }

  ::logstash::configfile { 'output-elasticsearch':
    content => template('cikl_logstash/logstash-output-event.conf.erb'),
    require => File['elasticsearch-cikl-template'],
    order   => 30
  }

  logstash::configfile { 'output-resolve':
    content => template('cikl_logstash/logstash-output-resolve.conf.erb'),
    order   => 40
  }
}
